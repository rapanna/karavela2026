<?php
/**
 * Šablona stránky „Last Minute" (slug: last-minute).
 * Vypisuje jednotlivé budoucí termíny, které mají ručně zadanou
 * Last Minute cenu (tm_cena_lm_p), seřazené podle data odjezdu.
 */
...
if ( $q->have_posts() ) {
    while ( $q->have_posts() ) {
        ...
        foreach ( $od as $i => $od_val ) {
            ...
            $lm_val = $lm[ $i ] ?? '';
            if ( ! is_numeric( $lm_val ) || (int) $lm_val <= 0 ) continue; // jen termíny s LM cenou
...
<main role="main" class="content content--subpage">
  <div class="container">
    ...
    <div class="row recommended-trips">
      ...
      <?php endforeach; else : ?>
        <div class="col-sm-12"><p>Momentálně nejsou vypsány žádné Last Minute termíny.</p></div>
      <?php endif; ?>
    </div>
